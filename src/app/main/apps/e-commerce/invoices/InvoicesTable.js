...
function InvoicesTable(props) {
	...
	const searchText = useSelector(({ eCommerceApp }) => eCommerceApp.invoices.searchText);
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			setData(_.filter(invoices, item => item.name.toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
			setData(invoices);
		}
	...
	function handleClick(item) {
		...
		props.history.push(`/apps/e-commerce/invoices/${item.id}/${item.name}`);

	}
	const deleteAccount = (accountt) => {
		console.log("check", accountt.id);
		const deleteid = accountt.id;
		axios.delete('https://hacorps.tech/estate92/public/api/invoices/'+deleteid)
      .then(res => {
        console.log(res);
